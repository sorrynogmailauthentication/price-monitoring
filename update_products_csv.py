...
def update_products_csv(filename='all_products.csv'):
    all_pairs = []
    for d in [LENTA_PRODUCT_LIST_DICT, VKUSVIL_PRODUCT_LIST_DICT, PEREKRESTOK_PRODUCT_LIST_DICT,
            DIXY_PRODUCT_LIST_DICT, PYATEROCHKA_PRODUCT_LIST_DICT, AUCHAN_PRODUCT_LIST_DICT]:
        for product_name, product_url in d.items():
            all_pairs.append((product_name, product_url))
    ...
    if os.path.exists(filename):
        ...
        # Find new products
        new_products = []
        for product_name, product_url in all_pairs:
